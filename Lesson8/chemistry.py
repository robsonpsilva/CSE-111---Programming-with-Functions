...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    total_molar_mass = 0
    for sqt in symbol_quantity_list:
        symbol = sqt[SYMBOL_INDEX]
        quantity = sqt[QUANTITY_INDEX]
        name_atomic_mass = periodic_table_dict[symbol]
        name = name_atomic_mass[NAME_INDEX]
        atomic_mass = name_atomic_mass[ATOMIC_MASS_INDEX]
        total_molar_mass += quantity * atomic_mass 

    return total_molar_mass

def compute_number_of_moles(molecule_mass, sample_mass):
    #Calculate the number of moles in the sample
    sample_mole = sample_mass / molecule_mass
    return sample_mole
...
def main():
   # Get a chemical formula for a molecule from the user.
    ...
    # Compute the number of moles in the sample.
    total = compute_number_of_moles(total_molar_mass, mass_in_grams)

    # Print the molar mass.
    print(f'{total_molar_mass:.5f} grams/mole')

    # Print the number of moles.
    print(f'{total:.5f} moles')
